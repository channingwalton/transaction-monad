...
import com.casualmiracles.transaction.{Result, TransactionF}
...
class TransactionIOTest extends FreeSpec with MustMatchers with EitherValues {
  ...
  "Success" in {
    assertTransaction(
      ...
      Result.Success[String, String]("hi"), true, false)
  }
  ...
  "Failure" - {
    "when the result is a failure" in {
      assertTransaction(
        ...
        Result.Failure[String, String]("oops"), false, true)
    }
    ...
    "when a non-fatal exception is thrown" in {
      ...
      assertTransaction(
        ...
        Result.Error(exception), false, false)
    }
  }

  private def assertTransaction[E, A](trans: TransactionF[IO, E, A], result: Result[E, A], onSuccessRun: Boolean, onFailureRun: Boolean) = {
    var onSuccess = false
    var onFailure = false
